...
class WorkJobDetailPage extends StatelessWidget {
      ...
      actions: job == null
          ...
          : [
              IconButton(
                ...
                onPressed: job.status == JobStatus.done ? null : () => _confirmDone(context, repo, job),
              ),
              IconButton(
                tooltip: 'แก้ไข',
  ...
  Future<void> _confirmDone(BuildContext context, JobsRepository repo, WorkJob job) async {
    ...
    double? extraPaid;
    try {
      extraPaid = await showDialog<double>(
        context: context,
        builder: (context) => AlertDialog(
          backgroundColor: _card,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'ยอดชำระเพิ่มจากมัดจำ (บาท)',
                ),
              ),
            ],
          ),
